...
def fetch_ranking(days: int) -> dict[str, dict]:
    ...
    for row in parser.rows:
        ...
        if match:
            found[match.group(2)] = {"name": match.group(3), "rank": int(match.group(1))}
    if not found:
...
def build_report(rankings: dict[int, dict[str, dict]]) -> dict:
    ...
    for code, observations in seen.items():
        ...
        record = {"code": code, "name": observations[0]["name"], "windows": [f"{day}d" for day in days], "appearances": len(observations), "avg_rank": round(avg_rank, 1), "latest_window": f"{min(days)}d"}
    ...
    return {"generated_at": datetime.now(timezone(timedelta(hours=8))).isoformat(), "source_urls": {f"{day}d": BASE_URL.format(days=day) for day in WINDOWS}, "themes": themes}
